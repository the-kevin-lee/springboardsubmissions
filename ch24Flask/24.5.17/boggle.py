...
class Boggle():
    ...
    # constructor function that will set class attribute of 'words'
    # to the words that are rendered from 'words.txt'
    def __init__(self):
    ...
    # parses through the words from 'dict_path' upon opening it
    # strips each word and then returns those words

    # WHERE IS THIS DICT_PATH? 
    ...
    # HOW DOES THIS CODE WORK TO CREATE A 5X5 BOARD?
    def make_board(self):
    ...
    # Checks to see if chosen word is a valid word which needs to meet 2 criteria:
    # 1) If the word is provided within the dictionary
    # 2) If the word is within the board (that is created)
    ...
    def find_from(self, board, word, y, x, seen):
        ...
        # will exit if the length of the matrix is longer than 4 (from 0 --> 4)
        if x > 4 or y > 4:
            return
